...
def grab_all_tasks_in_project(asana_client, asana_config, start_date):
    task_params = {
        "project": [asana_config['project_id_pulling']],
...
def days_data_for_tasks(asana_config, tasks, start_date, end_date):
        ...
        if days_to_close > std_above:
            link = (
                "https://app.asana.com/0/{project_id}/{task_id}/ : {}".format(
                    days_to_close,
                    project_id=[asana_config['project_id_pulling']],
                    task_id=t["id"]))
...
def post_all_in_asana_task(
    ...
    name = "{} Results - {}".format(
        asana_config['project_name_pulling'], todays_date_str)
    task_details = (task_information + tasks_stats)

    params = {
        "workspace": asana_config['workspace_id'],
        "projects": [asana_config['project_id_posting']],
...
def main():
    ...
    tasks = grab_all_tasks_in_project(
        asana_client, asana_config, args.start_date)
    tasks_stats = days_data_for_tasks(
        asana_config, tasks, args.start_date, args.end_date)
